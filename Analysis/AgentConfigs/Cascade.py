...
def net_cfg(space_description, init: bool, prop_val: bool, prop_action: bool, fallback_init: float, actor_hidden: tuple):
...
def agent_cfg(space_descr: EnvSpaceDescription, base_steps: int, propagate_value: bool, propagate_action: bool, fallback_coef: float, train_only_top: bool,
              fb_init: float, sequential: bool, stacks: int, actor_hidden: tuple[int], cyclical_lr: bool, continuous: bool = False):
    ...
    init_net_conf = net_cfg(space_descr,True, prop_val=propagate_value, prop_action=propagate_action, fallback_init=fb_init, actor_hidden=actor_hidden)
    stacked_net_conf = net_cfg(space_descr, False, prop_val=propagate_value, prop_action=propagate_action, fallback_init=fb_init, actor_hidden=actor_hidden)
...
def agent(space_descr: EnvSpaceDescription,base_steps: Union[int,str] =1000000, propagate_value: Union[bool,str] = False, propagate_action: Union[bool,str] = False, fallback_coef: Union[float,str] = 0.0,
          ...
          actor_hidden: Union[tuple[int],str] = (16,16), cyclical_lr: Union[bool,str] = True, continuous: bool = False, continuation: str = None):
    return (lambda: Cascade(cfg = agent_cfg(space_descr, base_steps=int(base_steps), propagate_value=parse_bool(propagate_value), propagate_action=parse_bool(propagate_action),
                                            ...
                                            sequential=parse_bool(sequential), stacks=int(stacks), actor_hidden=parse_tuple(actor_hidden, lambda x: int(x)),cyclical_lr=parse_bool(cyclical_lr), continuous=parse_bool(continuous)))) if continuation is None else lambda: Agent.load(Path(continuation))
