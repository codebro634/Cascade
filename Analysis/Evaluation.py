...
from Agents.CombNHierarchical import HierComb
...
def evaluate_agent(agent: Agent,env: gym.core.Env, num_runs:int = 10, horizon_length:int = None, measure_return: bool = False, track_action_freqs: bool = False,
                   measure_fallback_stats: bool = False, cascade_net:ActorCriticCascade = None, measure_expected_state: bool = False, measure_entropies: bool = False,
                   comb_net:CombActorHead = None, verbose: bool = False):
    """
    ...
    :param measure_entropies: If set, returns the average horizontal and vertical entropy and total sum of attentions per action-net of the chooser in comb_net
    :param comb_net: Mandtory if measure_entropies is set. This net is then used for these measurements
    """
    ...
    #Optional measurements setup
    if measure_entropies:
        assert isinstance(comb_net, CombActorHead)
        vent_sum, net_attentions = 0, np.array([0] * comb_net.n, dtype=float)
    ...
    for _ in range(num_runs):
        ...
        while not done and (not horizon_length or steps < horizon_length):
            ...
            if measure_entropies:
                with torch.no_grad():
                    tmp = comb_net.get_action(torch.Tensor(obs).unsqueeze(0))
                    vent_sum += tmp["vent"].item()
                    net_attentions += np.array(tmp["p"].squeeze(0))
    ...
    if measure_entropies:
        measurements.update({"attentions_sum": net_attentions, "vent":vent_sum/total_obs, "hent":Categorical(probs=torch.Tensor(net_attentions/ total_obs)).entropy().item() })
    if measure_expected_state:
...
def measure_base_nets(agent: Agent, env: gym.core.Env, num_runs: int = 10) -> dict:
    """Loads agents and environment from file then measures performances of an agents base nets. Assume the agent is either CombN or Cascade

        The instances environments generated by calling env are not closed!
    """

    # Check if agent is either CombN or Cascade
    assert (isinstance(agent, PPO) and isinstance(agent.net.actor, CombActorHead)) or isinstance(agent,Agents.Cascade.Cascade)\
           or (isinstance(agent,Agents.CombNHierarchical.HierComb) and isinstance(agent.current_cycle_agents[0].net.actor,CombActorHead))


    is_ppo = isinstance(agent, PPO)
    is_hier = isinstance(agent,Agents.CombNHierarchical.HierComb)
    is_casc = isinstance(agent,Agents.Cascade.Cascade)

    n_base = agent.net.actor.n if is_ppo else (len(agent.top.net.cascade) if is_casc else agent.current_cycle_agents[0].net.actor.n)
    current_return = np.zeros(shape=(n_base,))
    for i in range(n_base):
        if is_ppo:
            agent.net.actor.regress_to_one_action_net(i)
        elif is_casc:
            agent.top.net.regress_to_one_base(i)
        elif is_hier:
            agent.current_cycle_agents[0].net.actor.regress_to_one_action_net(i)
...
#Measures the average action distribution and the average maximum action probability of all models in run_group (This assumes that the saved env has a discrete action-space)
def measure_action_distribution_of_discrete_chooser(run_group: Path, num_runs: int =10):
    print(f"Action distributions of group {run_group}:")
    args_dict = lambda agent: {"measure_return": True, "track_action_freqs": True}
    evaluate_run_group(run_group, args_dict, num_runs=num_runs, ignore=["distribution"], verbose=True)

#Measures the average horizontal and vertical entropy of all models in run_group (This assumes CombN-models are saved in run_group)
def measure_entropies(run_group: Path, num_runs: int = 10):
    print(f"Entropies of group {run_group}:")
    args_dict = lambda agent: {"measure_return": True, "measure_entropies": True, "comb_net":agent.current_cycle_agents[0].net.actor if agent.__class__ == HierComb else agent.net.actor }
    evaluate_run_group(run_group, args_dict, num_runs=num_runs, ignore=["attentions_sum"], verbose=True)
...
#Measures the average best performance of all base-nets for all models in run_group (This assumes that these models in run_group are either CombN or Cascade models)
