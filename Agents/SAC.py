...
#             else:
#                 with torch.no_grad():
#                     actions, _, _ = actor.get_action(torch.Tensor(obs).to(device), deterministic=False, target=False, eval_mode=True)
#
...
#
#                 qf1_a_values = qf1(data_observations, data.actions).view(-1)
#                 qf2_a_values = qf2(data_observations, data.actions).view(-1)
